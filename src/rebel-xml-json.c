/* rebel-xml-json.c - Rebel XML and JSON interface
...
CELL *p_XMLerror(CELL *params)
{
    CELL *errorCell;
    CELL *cell;
    (void)params; /* ufko was unused param warning */
    ...
    while(!xmlError && !closingFlag)
    {
        ...
        if(tagPos > 0)
        {
            if( (tagStack != NULL) || (node->contents != (UINT)nilCell))
            {
                ...
                {
                    lastNode = appendNode(lastNode, makeTextNode(XML_TEXT, stuffStringN(source, tagPos)));
                }
            }
...
CELL *parseNormalTag(char *endSrc, char *newSrc)
{
    ...
    while((unsigned char)*source <= ' ' && source < endSrc)
    {
...
CELL *parseAttributes(char *endSrc)
{
    ...
    while(!xmlError && source < endSrc)
    {
        while((unsigned char)*source <= ' ' && source < endSrc)
        {
            source++;    /* strip leading space */
        }
        namePos = source;
        nameLen = 0;
        while((unsigned char)*source > ' ' && *source != '=' && source < endSrc)
        {
            source++, nameLen++;    /* get end */
        }
        if(nameLen == 0)
        {
            break;
        ...
        valPos = source;
        valLen = 0;
        while(*source != quoteChar && source < endSrc)
        {
            source++, valLen++;
        }
        if(*source != quoteChar)
...
int isWhiteSpaceStringN(char *source, int tagPos)
{
    while(tagPos--) if((unsigned char)*source++ > 32)
        {
            return(FALSE);
        }
    return(TRUE);
...
CELL *getJSONstring(char *jsonStr, char * * restStr)
{
    ...
    if(*jsonStr != '"')
    {
        return(setJSONerror(ERR_JSON_MISSING_KEY, jsonStr));
    }
    ...
    if(*jsonStr++ != '"')
    {
        return(setJSONerror(ERR_JSON_CLOSING_QUOTE, jsonStr));
    }
...
CELL *getJSONvalue(char *jsonStr, char * * restStr)
{
    ...
    {
        goto NOT_STRING_OR_NUMBER;
    }

    number = alloca(32);
    while(isDigit((unsigned char)*jsonStr))
    {
        ++jsonStr;
    }
    if(*jsonStr == '.')
