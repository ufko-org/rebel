...
CELL *p_close(CELL *params)
{
    ...
    if(handle == printDevice)
    {
        printDevice = 0;
    }
    if(deleteIOsession(handle))
    {
        return(trueCell);
    }
    return(nilCell);
...
CELL *writeBuffer(CELL *params, int lineFeed)
{
    ...
    if(isNumber(device->type))
    {
        ...
        if(lineFeed)
            if(write((int)handle, LINE_FEED, LINE_FEED_LEN) == -1)
            {
                return(nilCell);
            }
    }
...
CELL *p_readLine(CELL *params)
{
    ...
    while(TRUE)
    {
        if((bytesRead = read((int)handle, &chr, 1)) <= 0)
        {
            break;
        }
        if(chr == '\n')
...
CELL *p_process(CELL *params)
{
    ...
    if(forkResult == 0)
    {
        ...
        {
            close(STDIN_FILENO);
            if(dup2((int)inpipe, STDIN_FILENO) == -1)
            {
                exit(0);
            }
            close((int)inpipe);
        ...
        {
            close(STDOUT_FILENO);
            if(dup2((int)outpipe, STDOUT_FILENO) == -1)
            {
                exit(0);
            }
            if(!errpipe)
                if(dup2((int)outpipe, STDERR_FILENO) == -1)
                {
                    exit(0);
                }
            close((int)outpipe);
        ...
        {
            close(STDERR_FILENO);
            if(dup2((int)errpipe, STDERR_FILENO) == -1)
            {
                exit(0);
            }
            close((int)errpipe);
...
char *getUUID(char *str, char *node)
{
    ...
    if(node == NULL)
    {
        nodeID[0] = (long int)arc4random();
        nodeID[1] = (long int)arc4random();
        nodeID[2] = (long int)arc4random();
        uuid_version = 4;
    ...
    {
        clock_seq = (long int)arc4random();
        uuid.time_low = (long int)arc4random();
        uuid.time_mid = (long int)arc4random();
        uuid.time_hi_and_version = (long int)arc4random();
    }
    else
    {
...
SYMBOL *getSymbolCheckProtected(CELL *params)
{
    ...
    {
        errorProcExt(ERR_SYMBOL_EXPECTED, params);
    }
