/* rebel-web.c --- HTTP network protocol routines for Rebel
...
ssize_t sendf(int sock, int debug, char *format, ...)
{
    ...
    if(debug)
    {
        varPrintf(OUT_CONSOLE, "%s", buffer);
    }
...
CELL *getPutPostDeleteUrl(char *url, CELL *params, int type, int timeout)
{
    ...
    if(bufflen < MAX_URL_LEN + 1)
    {
        bufflen = MAX_URL_LEN + 1;
    }
...
CONNECT_TO_HOST:
    ...
    {
        sendf(sock, debugFlag, "User-Agent: Rebel v%d\r\n", version);
    }
    ...
    {
        if(type == HTTP_PUT_APPEND)
        {
            sendf(sock, debugFlag, "%s", "Pragma: append\r\n");
        }
        if(customHeader == NULL)
        {
        ...
        if(debugFlag)
        {
            varPrintf(OUT_CONSOLE, "%s", putPostStr);
        }
    }
...
READ_RESPONSE:
    ...
    while(strcmp(buff, "\r\n") != 0 && strcmp(buff, "\n") != 0)
    {
        ...
        if(listFlag || headRequest)
        {
            appendCellString(headerCell, buff, strlen(buff));
        }
...
size_t parseValue(char *str)
{
    while(!isDigit((unsigned char)*str) && *str != 0)
    {
        ++str;
    }
    return atol(str);
...
/* Base64 encoding/decoding
   ...
   this file from the cURL project is included in rebel-web.c for the
   Rebel functions 'base64-enc' and 'base64-dec'
...
size_t Curl_base64_decode(const char *src, char *dest)
{
    ...
    if(equalsTerm > 3)
    {
        equalsTerm = 3;    /* LM added 2006-09-08 */
    }
...
{
    return(base64(params, BASE64_ENC));
}
CELL *p_base64Dec(CELL *params)
{
    return(base64(params, BASE64_DEC));
}
