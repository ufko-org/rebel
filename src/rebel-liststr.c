/* rebel-liststr.c --- Rebel primitives handling lists and strings
...
/* ufko
extern void printResultStack(void);
*/
...
size_t adjustNegativeIndex(ssize_t index, size_t length)
{
    ...
    {
        errorProc(ERR_STRING_INDEX_INVALID);
    }
...
CELL *p_member(CELL *params)
{
    ...
    else if (list->type == CELL_STRING)
    {
        ...
        else
        {
            ...
            if(pos != -1)
            {
                return(stuffString((char *)list->contents + pos));
            }
        }
        return(nilCell);
    }
...
CELL *p_extend(CELL *params)
{
    ...
    {
        return(errorProcExt(ERR_LIST_OR_STRING_EXPECTED, target));
    }
...
CELL *p_select(CELL *params)
{
    ...
    while(params->type != CELL_NIL)
    {
        ...
        if(index < 0)
        {
            index = convertNegativeOffset(index, head);
        }
        if(index < idx)
        {
            list = head, idx = 0;
        }
        while(idx < index  && list != nilCell)
        {
            list = list->next, idx++;
        }
        if(list == nilCell)
...
{
    return startsEndsWith(params, STARTS_WITH);
}
CELL *p_endsWith(CELL *params)
{
    return startsEndsWith(params, ENDS_WITH);
}
...
CELL *p_rotate(CELL *params)
{
    ...
    if(cell->next != nilCell)
    {
        getInteger(cell->next, (UINT *)&count);
    }
    else
    {
        count = 1;
