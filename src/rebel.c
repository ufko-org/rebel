/* rebel.c --- entry point and main functions for Rebel
...
char linkOffset[] = "&&&&@@@@";
extern char preLoad[]; /* ufko: userland bootstrap */
...
/* set REBELDIR only if not set already */
void initRebelDir(void)
{
    if(getenv("REBELDIR") == NULL)
    {
        setenv("REBELDIR", REBELDIR, TRUE);
    }
}
    ...
    for(idx = 1; idx < argc; idx++)
    {
        if(strcmp(argv[idx], "-q") == 0)
        {
            ...
            for(j = 1; j < argc; j++)
            {
                if(argv[j][0] != '-')
                {
                    loadFile(argv[j], 0, 0, mainContext);
                }
            }
            exit(0);
            /* end of -q handling */
...
void printHelpText(void)
{
    ...
    varPrintf(OUT_CONSOLE,
              ...
              " -http-safe safe mode",
              " -6 IPv6 mode",
              "\nmore info at https://rebel-lang.org");
}
...
void setupServer(int reconnect)
{
    ...
    {
        printf("Rebel server setup on %s failed.\n", IOdomain);
        exit(1);
    }
...
EXEC_COMMANDLINE:
    if(noPromptMode == FALSE && *command == '!' && *(command + 1) != ' ' && strlen(command) > 1)
    {
        if(system(command + 1))
        {
            return;    /* avoid stupid compiler warning */
        }
        return;
    }
    ...
    if(cmdStream != NULL && batchMode)
    {
        ...
        for(;;)
        {
            ...
            else if(fgets(buff, MAX_COMMAND_LINE - 1, IOchannel) == NULL)
            {
                break;
            }
...
int executeSymbol(SYMBOL *symbol, CELL *params, CELL * * result)
{
    ...
    if(symbol == nilSymbol || symbol == trueSymbol || symbol == NULL)
    {
        return(0);
    }
...
CELL *evaluateExpression(CELL *cell)
{
    ...
    if(traceFlag)
    {
        traceExit(result, cell, pCell, args);
    }
    --recursionCount;
    return(result);
}
...
void allocBlock(void)
{
    ...
    {
        printErrorMessage(ERR_OUT_OF_MEMORY, NULL, 0);
...
void *allocMemory(size_t nbytes)
{
    ...
    {
        fatalError(ERR_OUT_OF_MEMORY, NULL, 0);
    }
...
void printCell(CELL *cell, UINT printFlag, UINT device)
{
    ...
    {
        case CELL_NIL:
            varPrintf(device, "nil");
            break;

        case CELL_TRUE:
            varPrintf(device, "true");
            break;
...
void printFn(SYMBOL *sPtr, UINT device)
{
    ...
    while((cell = cell->next) != nilCell)
    {
        ...
        printCell(cell, TRUE, device);
        if(!(cell->type & ENVELOPE_TYPE_MASK) && cell->next != nilCell)
        {
            varPrintf(device, " ");
        }
    }
...
{
    char str[48];
    size_t maxlen = sizeof(str) - 1 - 3; /* 3 chars for "..." */
    snprintf(str, sizeof(str), "...%.*s", (int)maxlen,
             ((stream->ptr - stream->buffer) > (int)maxlen
              ? stream->ptr - maxlen
              : stream->buffer));
    errorProcExt2(ERR_MISSING_PAR, stuffString(str));
}
...
CELL *errorProcAll(int errorNumber, CELL *expr, int deleteFlag)
{
    if(!traceFlag)
    {
        fatalError(errorNumber, expr, deleteFlag);
    }
...
void printErrorMessage(UINT errorNumber, CELL *expr, int deleteFlag)
{
    ...
    {
        tracePrint(errorStream.buffer, NULL);
    }
}
...
CELL *getInteger(CELL *params, UINT *number)
{
    ...
    else if(cell->type == CELL_FLOAT)
    {
        ...
        else if(*(double *)&cell->contents >  9223372036854775807.0)
        {
            *number = 0x7FFFFFFFFFFFFFFFLL;
        }
        else if(*(double *)&cell->contents < -9223372036854775808.0)
        {
...
{
    return(evalString(params, EVAL_STRING));
}
CELL *p_readExpr(CELL *params)
{
    return(evalString(params, READ_EXPR));
}
...
CELL *sysEvalString(char *evalString, SYMBOL *context, CELL *proc, int mode)
{
    ...
    while(TRUE)
    {
        ...
        {
            if(resultCell != nilCell)
            {
                pushResult(resultCell);    /* 10.6.3 */
            }
...
CELL *defineOrMacro(CELL *params, UINT cellType, int flag)
{
    ...
    {
        symbol = (SYMBOL *)argsPtr->contents;
    }
...
{
    return(let(params, LET_STD));
}
CELL *p_letn(CELL *params)
{
    return(let(params, LET_NEST));
}
CELL *p_letExpand(CELL *params)
{
    return(let(params, LET_EXPAND));
}
CELL *p_local(CELL *params)
{
    return(let(params, LET_LOCAL));
}
...
CELL *implicitNrestSlice(CELL *num, CELL *params)
{
    ...
    else if(isList(list->type))
    {
        ...
        if(n < 0)
        {
            n = convertNegativeOffset(n, list);
        }
...
{
    return(repeat(params, REPEAT_WHILE));
}
CELL *p_doWhile(CELL *params)
{
    return(repeat(params, REPEAT_DOWHILE));
}
CELL *p_until(CELL *params)
{
    return(repeat(params, REPEAT_UNTIL));
}
CELL *p_doUntil(CELL *params)
{
    return(repeat(params, REPEAT_DOUNTIL));
}
...
CELL *p_copy(CELL *params)
{
    ...
    /* experimental: copy a cell from address, from:
       http://www.rebelfanclub.alh.net/forum/viewtopic.php?f=5&t=4548
...
void saveSymbols(SYMBOL *sPtr, UINT device)
{
    ...
    if(sPtr != NIL_SYM && sPtr != NULL)
    {
        ...
        if(type == CELL_CONTEXT)
        {
            ...
            {
                if(sPtr != currentContext && *sPtr->name != '$')
                {
                    saveContext(sPtr, device);
                }
            }
            else
            {
...
CELL *p_isAtom(CELL *params)
{
    ...
    params = evaluateExpression(params);
    if(params->type & ENVELOPE_TYPE_MASK)
    {
        return(nilCell);
    }
    return(trueCell);
...
CELL *p_reset(CELL *params)
{
    ...
    if(params != nilCell)
    {
        ...
        if(isNumber(params->type))
        {
            ...
            if(MAX_CELL_COUNT < MAX_BLOCK)
            {
                MAX_CELL_COUNT = MAX_BLOCK;
            }
...
CELL *setEvent(CELL *params, SYMBOL * * eventSymPtr, char *sysSymName)
{
    if(params != nilCell)
    {
        getCreateSymbol(params, eventSymPtr, sysSymName);
    }
...
CELL *p_mainArgs(CELL *params)
{
    ...
    if(params != nilCell && cell->type == CELL_EXPRESSION)
    {
        ...
        if(idx < 0)
        {
            idx = convertNegativeOffset(idx, (CELL *)cell);
        }
        while(idx--)
        {
            cell = cell->next;
