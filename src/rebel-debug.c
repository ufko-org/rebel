/* rebel-debug.c --- debugging functions, and functions to trap signals and timers
...
void tracePrint(char *label, CELL *expr)
{
    ...
    if(expr)
    {
        printCell(expr, TRUE, OUT_DEVICE);
    }
...
void traceEntry(CELL *cell, CELL *pCell, CELL *args)
{
    ...
    if(traceFlag & (TRACE_IN_ENTRY | TRACE_IN_EXIT | TRACE_DEBUG_NEXT))
    {
        return;
    }
...
{
    if(traceFlag & (TRACE_IN_ENTRY | TRACE_IN_EXIT | TRACE_SIGNAL | TRACE_SIGINT | TRACE_TIMER))
    {
        return;
    }
...
int debugPrintFunction(CELL *cell)
{
    ...
    if(strPos != NULL)
    {
        ...
        while(*(strPos + preLen + pos) <= ' ' && *(strPos + preLen + pos) != 0)
        {
            ++pos;
        }
