    ...
    switch(pattern->type)
    {
        ...
        case CELL_SYMBOL:
            if(pattern->contents == (UINT)questionSymbol)   /* '?' */
            {
                ...
                if(!flag)
                {
                    matches = linkMatches(&matchList, matches, copyCell(list));
                }
                break;
            }
...
CELL *p_assoc(CELL *params)
{
    ...
    while(key != nilCell)
    {
        ...
        if(!listMode || (key = key->next) == nilCell)
        {
            break;
        }
...
CELL *p_count(CELL *params)
{
    ...
    if(vectorList)
    {
        cell = resortVectorToList(vectorList, lengthList);
    }
...
CELL *implicitIndexList (CELL *list, CELL *params)
{
    ...
    {
        return(errorProcExt(ERR_LIST_INDEX_INVALID, params));
    }
    ...
    while(isList(list->type))
    {
        ...
        */
        ...
        if(params == nilCell || !isList(list->type))
        {
            break;
        }
         ...
         CELL * * next, REFSTACK *refStack, int mode)
{
    ...
    while(list != nilCell)
    {
        if(compareFunc(keyCell, list, funcCell) == 0)
        {
            if(refStack->base)
            {
                if(refStack->idx < MAX_REF_STACK)
                {
                    pushRef(idx);
                }
                else
                {
                    errorProc(ERR_NESTING_TOO_DEEP);
                }
...
CELL *p_array(CELL *params)
{
    ...
    while(params != nilCell && p < 17)
    {
        ...
        {
            return(errorProcExt(ERR_NUMBER_EXPECTED, list));
        }
        params = params->next;
    }
...
CELL *makeArray(ssize_t *index, int p)
{
    ...
    /* ^ufko OK: array->aux holds byte size of array storage, not element count */
...
CELL *implicitIndexArray(CELL *cell, CELL *params)
{
    ...
    {
        return(errorProcExt(ERR_ARRAY_INDEX_OUTOF_BOUNDS, params));
    }
    ...
    while(cell->type == CELL_ARRAY)
    {
        ...
        cell = *(addr + index);
        if(params == nilCell || cell->type != CELL_ARRAY)
        {
            break;
        }
        params = getIntegerExt(params, (UINT *)&index, evalFlag);
