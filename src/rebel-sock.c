...
CELL *p_netConnect(CELL *params)
{
    ...
    char *remoteHostName;
    UINT portNo;
    UINT topt = CONNECT_TIMEOUT; /* default ms timeout from rebel.h */
    int sock;
    int type;
    int protocol = 0;
...
int getIpPortFromSocket(int sock, int peerLocalFlag, char *IPaddress)
{
    ...
    *IPaddress = 0;
    ...
    else if(getpeername(sock, address_sin, (void *)&address_sin_len) == SOCKET_ERROR)
    {
        return(SOCKET_ERROR);
    }
...
CELL *p_netReceiveUDP(CELL *params)
{
    ...
    if(wait > 0)
    {
        if((elapsed = wait_ready(sock, wait, READY_READ)) <= 0)
        {
            ...
            if(elapsed == 0)
            {
                return(netError(ERR_INET_TIMEOUT));
            }
            else
            {
...
CELL *p_netSelect(CELL *params)
{
    ...
    {
        return(errorProcExt(ERR_LIST_OR_NUMBER_EXPECTED, params));
    }
    ...
    {
        return(errorProcExt2(ERR_INVALID_PARAMETER, stuffString(mode)));
    }
...
void writeLog(char *text, int newLine)
{
    ...
    if(write(handle, text, strlen(text)) < 0)
    {
        return;
    }
    if(newLine)
        if(write(handle, &LINE_FEED, LINE_FEED_LEN) < 0)
        {
            return;
        }
    close(handle);
...
FILE *serverFD(int port, char *domain, int reconnect)
{
    ...
    if(!reconnect)
    {
        ...
        {
            snprintf(text, 79, "Rebel v.%d listening on %s", version, domain);
            writeLog(text, TRUE);
        }
    }
    else
...
CELL *ping(CELL *address, int maxwait, int listmode, int maxCount, int flag)
{
    ...
    while(address != nilCell)
    {
        ...
        for(ipNo = startIp; ipNo <= endIp; ipNo++)
        {
            ...
            while(wait_ready(s, 10000, READY_WRITE) <= 0)
            {
                ...
                if((timeout = (timediff_ms(tp, tv) > maxwait)))
                {
                    break;
                }
                continue;
    ...
    while(sendCount)
    {
        ...
        if(ADDR_FAMILY == AF_INET6)
        {
            ...
            if(icp6->icmp6_id != (getpid() & 0xFFFF))
            {
                continue;
            }
