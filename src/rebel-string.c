...
{
    return(strUpperLower(params, STR_UPPER));
}
CELL *p_lower(CELL *params)
{
    return(strUpperLower(params, STR_LOWER));
}
CELL *p_title(CELL *params)
{
    return(strUpperLower(params, STR_TITLE));
}
...
ssize_t searchBuffer(char *buffer,  size_t length, char *string, size_t size, int caseFlag)
{
    ...
    if(caseFlag == FALSE)
    {
        while(position < length)
        {
            if(toupper(*buffer) == toupper(*string))
                if(my_strnicmp(buffer, string, size) == 0)
                {
                    break;
                }
            position++;
    ...
    else /* case sensitive */
    {
        ...
        {
            if(*buffer == *string)
                if(memcmp(buffer, string, size) == 0)
                {
                    break;
                }
            position++;
...
CELL *p_getString(CELL *params)
{
    ...
    if(len == 4 || len == 2) if(memcmp(limit, zeros, len) == 0)
        {
            inc = len;
        }
    while(offset < size)
    {
        if(offset < (size - len) &&
                memcmp((char *)(address + offset), limit, len) == 0)
        {
            break;
        }
        offset += inc;
...
CELL *p_parse(CELL *params)
{
    ...
    while(string != NULL)
    {
        ...
        {
            if(getToken(&stream, token, &tklen) == TKN_EMPTY)
            {
                break;
            }
...
CELL *p_trim(CELL *params)
{
    ...
    {
        ptr = str;
        while((unsigned char)*str <= 32 && left < len)
        {
            str++, left++;
        }
        if(left == len)
        ...
        str = ptr + len - 1;
        while((unsigned char)*str <= 32 && right < len)
        {
            str--, right++;
        }
...
/* PCRE options from manual not allowed from Rebel regex options in pcre_compile() */
            ...
            switch (buffer[i])
            {
                ...
                case 'u':
                    *options |= 2048;
                    break;      /* PCRE_UTF8 */
                default:
                    return(errorProcExt(ERR_INVALID_OPTION, cell));
            }
        }
    }
    else
